  ...
  def show
    ...
    redirect_to magic_set_path(@set) and return if @object.disabled
      ...
      @bids = Transaction.find_by_sql("SELECT t.price, t.magic_card_id, t.status,
        ...
        FROM Transactions AS t WHERE t.magic_card_id = #{@object.id} AND t.status = 'buying' GROUP BY t.price, t.magic_card_id, t.status ORDER BY t.price DESC").first(5)
      @listings = Transaction.find_by_sql("SELECT t.price, t.magic_card_id, t.status,
        ...
        FROM Transactions AS t WHERE t.magic_card_id = #{@object.id} AND t.status = 'selling' GROUP BY t.price, t.magic_card_id, t.status ORDER BY t.price ASC").first(5)
      @sales = Transaction.find_by_sql("SELECT t.price, t.magic_card_id, t.status, DATE_TRUNC('minute', t.finish) AS truncated_finish,
        ...
        FROM Transactions AS t WHERE t.magic_card_id = #{@object.id} AND t.status = 'finished' GROUP BY t.price, t.magic_card_id, t.status, truncated_finish ORDER BY truncated_finish DESC").first(5)
      ...
      @graph_data[:avg] = []
      @graph_data[:volume] = []
      @date_range = (Date.new(2015,6,1)..Date.today).map{|date| date.strftime('%Q').to_i }
